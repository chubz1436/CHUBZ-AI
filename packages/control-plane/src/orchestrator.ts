...
export class M4Orchestrator {
  ...
  public claimNextDispatch(): DispatchCommand {
    return this.database.connection.transaction(() => {
      const rows = this.database.connection.prepare("SELECT task_id,attempt_id,operation_id,grant_id,queue_sequence FROM m4_dispatch_queue WHERE status='queued' ORDER BY queue_sequence").all() as Array<{ task_id: string; attempt_id: string; operation_id: string; grant_id: string; queue_sequence: number }>;
