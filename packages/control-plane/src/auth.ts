...
export class AuthService {
  ...
  async bootstrap(username: string, password: string, requestId: string): Promise<void> {
    if (!/^[a-zA-Z0-9_.-]{3,64}$/.test(username) || password.length < 12 || password.length > 256 || this.hasAdministrator()) throw genericFailure;
  ...
  async login(username: string, password: string, requestId: string): Promise<{ cookie: string; principal: Principal }> {
    ...
    const raw = token(); const csrf = token(); const now = Date.now(); const idHash = hash(raw); const expires = new Date(now + this.config.sessionTtlMs).toISOString(); const idle = new Date(now + this.config.sessionIdleMs).toISOString();
    this.database.connection.transaction(() => {
      this.database.connection.prepare("DELETE FROM sessions WHERE administrator_id = ? AND revoked_at IS NULL").run(row.id);
      this.database.connection.prepare("INSERT INTO sessions(id_hash, administrator_id, csrf_hash, created_at, expires_at, idle_expires_at, last_seen_at) VALUES (?, ?, ?, ?, ?, ?, ?)").run(idHash, row.id, hash(csrf), new Date(now).toISOString(), expires, idle, new Date(now).toISOString());
  ...
  authenticate(rawCookie: unknown): Principal | undefined {
    ...
    const csrf = token();
    this.database.connection.prepare("UPDATE sessions SET last_seen_at=?, idle_expires_at=? WHERE id_hash=?").run(new Date().toISOString(), new Date(Date.now() + this.config.sessionIdleMs).toISOString(), row.id_hash);
  ...
  verifyCsrf(principal: Principal, provided: unknown): boolean {
    ...
    if (!row) return false;
    const left = Buffer.from(row.csrf_hash); const right = Buffer.from(hash(provided));
    return left.length === right.length && timingSafeEqual(left, right);
  }
  revoke(rawCookie: unknown, requestId: string): void { if (typeof rawCookie === "string") { const session = this.database.connection.prepare("SELECT administrator_id FROM sessions WHERE id_hash=?").get(hash(rawCookie)) as { administrator_id: string } | undefined; this.database.connection.prepare("UPDATE sessions SET revoked_at=? WHERE id_hash=?").run(new Date().toISOString(), hash(rawCookie)); this.record("logout", session?.administrator_id, requestId); } }
  private record(kind: string, administratorId: string | undefined, requestId: string): void { this.database.connection.prepare("INSERT INTO auth_events(event_kind, administrator_id, occurred_at, request_id) VALUES (?, ?, ?, ?)").run(kind, administratorId ?? null, new Date().toISOString(), requestId); }
}
