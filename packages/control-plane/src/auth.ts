...
export class AuthService {
  ...
  async login(username: string, password: string, requestId: string): Promise<{ cookie: string; principal: Principal }> {
    ...
    const raw = token(); const csrf = token(); const now = Date.now(); const idHash = this.digest(raw); const expires = new Date(now + this.config.sessionTtlMs).toISOString(); const idle = new Date(now + this.config.sessionIdleMs).toISOString();
    this.database.connection.transaction(() => {
