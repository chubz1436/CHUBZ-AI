...
export const AdapterRunSchema = z.strictObject({
  ...
  if (value.captureRefs.length + value.evidenceRefs.length + value.runtimeProvenanceRefs.length > ADAPTER_COORDINATION_LIMITS.maxAggregateEntries) ctx.addIssue({ code: "custom", path: ["runtimeProvenanceRefs"], message: "run aggregate references exceed the M1F bound" });
});
...
const RelativePatternSchema = z.string().min(1).max(256).regex(/^[A-Za-z0-9][A-Za-z0-9._/*-]*$/).refine((value) => !value.includes("..") && !value.includes("//") && value !== "*" && value !== "/*" && !value.startsWith("*"), "must be a bounded relative pattern and cannot match everything");
...
export const LEASE_VALIDATION_CODES = Object.freeze(["VALID", "MALFORMED", "EXPIRED", "WRONG_HOLDER", "WRONG_ATTEMPT", "SUPERSEDED", "STALE_GENERATION", "CONFLICTING_EXCLUSIVE_LEASE", "RELEASE_BY_NON_HOLDER"] as const);
...
export const QuotaSnapshotSchema = z.strictObject({ coordinationVersion: z.literal(ADAPTER_COORDINATION_VERSION), quotaId: RefSchema, providerId: SlugIdSchema, adapterId: SlugIdSchema, workerId: SlugIdSchema, quotaKind: z.enum(["requests", "tokens", "credits", "concurrency", "unknown"]), remaining: z.number().finite().min(0).nullable(), used: z.number().finite().min(0).nullable(), limit: z.number().finite().positive().nullable(), resetAt: OptionalTime, window: z.enum(["minute", "hour", "day", "month", "unknown"]), source: QuotaSourceSchema, confidence: QuotaConfidenceSchema, observedAt: OptionalTime, expiresAt: OptionalTime, rateLimitState: z.enum(["clear", "limited", "unknown"]), circuitBreakerState: z.enum(["closed", "open", "half-open", "unknown"]), authenticationState: AuthenticationStateSchema, evidenceRefs: z.array(RefSchema).max(ADAPTER_COORDINATION_LIMITS.maxEvidence) }).superRefine((value, ctx) => { if ((value.source === "provider-reported" || value.source === "cli-observed") && !["validated", "observed"].includes(value.confidence)) ctx.addIssue({ code: "custom", path: ["confidence"], message: "observed source needs observed or validated confidence" }); if (value.source === "locally-estimated" && value.confidence !== "estimated") ctx.addIssue({ code: "custom", path: ["confidence"], message: "estimated source cannot claim validation" }); if (value.source === "owner-entered" && value.confidence !== "owner-attested") ctx.addIssue({ code: "custom", path: ["confidence"], message: "owner-entered quota is owner-attested, not provider-validated" }); if (value.source === "unknown" && value.confidence !== "unknown") ctx.addIssue({ code: "custom", path: ["confidence"], message: "unknown source requires unknown confidence" }); if (value.remaining !== null && value.limit !== null && value.remaining > value.limit) ctx.addIssue({ code: "custom", path: ["remaining"], message: "remaining cannot exceed limit" }); });
...
export const ASSIGNMENT_DISPATCH_CODES = Object.freeze(["DISPATCHABLE", "MALFORMED", "RECOMMENDATION_NOT_AUTHORIZATION", "CONFIRMATION_NOT_DISPATCH", "STALE", "UNBOUND_REFERENCE", "CROSS_SCOPE", "READINESS_NOT_READY", "CAPABILITY_NOT_READY", "QUOTA_NOT_USABLE", "EXPIRED_APPROVAL_GRANT"] as const);
export type AssignmentDispatchCode = (typeof ASSIGNMENT_DISPATCH_CODES)[number];
const AssignmentValidationContextSchema = z.strictObject({ now: IsoUtcTimestampSchema, readiness: AdapterReadinessSchema, quota: QuotaSnapshotSchema.nullable(), writeScope: WriteScopeSchema.nullable(), approvalGrant: CapabilityGrantSchema.nullable() });
...
export const EvidenceRecordSchema = z.strictObject({ coordinationVersion: z.literal(ADAPTER_COORDINATION_VERSION), evidenceId: RefSchema, kind: EvidenceKindSchema, subject: z.strictObject({ taskId: RefSchema, attemptId: RefSchema, operationId: RefSchema, adapterRunId: OptionalRef, leaseId: OptionalRef, artifactId: OptionalRef }), sourceId: RefSchema, authority: z.enum(["claim", "observed", "validated", "owner-attested", "system-derived"]), requiredBindings: z.array(z.enum(["task", "attempt", "operation", "adapter-run", "lease", "artifact", "grant", "journal"])).min(3).max(8), permittedUses: z.array(z.enum(["display", "routing", "readiness", "quota", "capture", "reconciliation", "approval-verification"])).min(1).max(7), prohibitedTrustElevation: z.boolean(), reference: RefSchema }).superRefine((value, ctx) => { if (!value.prohibitedTrustElevation) ctx.addIssue({ code: "custom", path: ["prohibitedTrustElevation"], message: "M1F evidence cannot elevate trust by label" }); if (value.kind === "owner-attested-manual-relay" && value.authority !== "owner-attested") ctx.addIssue({ code: "custom", path: ["authority"], message: "manual relay must remain owner-attested" }); if (value.kind === "automated-validation" && value.authority !== "validated") ctx.addIssue({ code: "custom", path: ["authority"], message: "automated validation needs validated authority" }); if (value.kind === "worker-claim" && value.authority !== "claim") ctx.addIssue({ code: "custom", path: ["authority"], message: "worker claim cannot self-elevate" }); if (["automated-observation", "health-capability-probe", "quota-observation", "cancellation-termination", "runtime-provenance"].includes(value.kind) && value.authority !== "observed") ctx.addIssue({ code: "custom", path: ["authority"], message: "observations must remain observed" }); if (["system-derived-state", "operation-journal", "recovery-reconciliation"].includes(value.kind) && value.authority !== "system-derived") ctx.addIssue({ code: "custom", path: ["authority"], message: "system-derived evidence must remain system-derived" }); if (value.requiredBindings.includes("adapter-run") && value.subject.adapterRunId === null) ctx.addIssue({ code: "custom", path: ["subject", "adapterRunId"], message: "adapter-run binding requires adapter run identity" }); if (value.requiredBindings.includes("lease") && value.subject.leaseId === null) ctx.addIssue({ code: "custom", path: ["subject", "leaseId"], message: "lease binding requires lease identity" }); if (value.requiredBindings.includes("artifact") && value.subject.artifactId === null) ctx.addIssue({ code: "custom", path: ["subject", "artifactId"], message: "artifact binding requires artifact identity" }); });
...
export const EVENT_DELIVERY_CLASSIFICATIONS = Object.freeze(["new", "duplicate", "conflicting-idempotency", "conflicting-cursor", "out-of-order"] as const);
export function classifyLifecycleDelivery(rawIncoming: unknown, rawRecorded: unknown): (typeof EVENT_DELIVERY_CLASSIFICATIONS)[number] { const incoming = parseLifecycleEvent(rawIncoming); const recorded = parseLifecycleEvent(rawRecorded); if (!incoming.ok || !recorded.ok) return "conflicting-idempotency"; if (incoming.value.cursor < recorded.value.cursor) return "out-of-order"; if (incoming.value.cursor === recorded.value.cursor && incoming.value.eventId === recorded.value.eventId && incoming.value.idempotencyKey === recorded.value.idempotencyKey) return "duplicate"; if (incoming.value.cursor === recorded.value.cursor) return "conflicting-cursor"; if (incoming.value.idempotencyKey === recorded.value.idempotencyKey) return "conflicting-idempotency"; return "new"; }
