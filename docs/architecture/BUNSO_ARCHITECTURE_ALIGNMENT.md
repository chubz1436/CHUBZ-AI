...
| M1F coordination contracts: assignment, write scope, lease, handoff, quota snapshot, evidence taxonomy, and the two proposed blocked reasons | Deferred to a named milestone | M1F, after M1E and before M2; each M1 subtask still requires its own explicit GO |
| Recommendation-first routing, auto-dispatch, dashboard/Kanban inspection surfaces, notifications, staging profile, and new M10/M11 milestones | Owner decision required | See owner decisions A-1 through A-4 below; none is silently adopted here |
...
| ID | Decision and choices | Why owner-only |
| --- | --- | --- |
| A-1 | Whether to evolve D-002 with Kanban/dashboard inspection surfaces that emit the same typed commands and approval cards, or retain the current chat-plus-panel layout. | It changes the accepted interaction model. |
| A-2 | Whether any routing may move beyond recommendation-first into category-specific auto-dispatch, or remain owner-confirmed. | It changes who initiates execution. |
| A-3 | Whether to add the draft's M10 routing/quota and M11 dashboard/notification milestones, and their position after the current Phase 1 sequence. | These are new product scope, not a correction to an accepted contract. |
| A-4 | Whether to approve a staging profile for the Command Center itself when runtime work reaches that point. | It creates an operational environment and must be separately designed. |
...
Chubz is the final approver. Bantay reviews strategy, safety, scope, and prompts. Codex is the current primary implementation worker. BUNSO remains the lead architecture-design source; Antigravity is secondary and capability-probed. Workers cannot override owner or repository policy, infer production authorization, or turn an informal draft into a governing contract.
