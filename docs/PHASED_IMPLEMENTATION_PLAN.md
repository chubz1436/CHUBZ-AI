...
> **M11 STATUS (2026-07-23):** M1A-M10 are accepted on `main` at `5bb492c5a107591d5c56da03b2b9919b3d0dfebc`. M11 has explicit owner GO only for the bounded local operational dashboard/alerts, production-shaped loopback runtime, strict configuration, Windows release bundle, safe process commands, diagnostics/support evidence, retention, and non-mutating upgrade preparation. Installation, deployment, remote access, service registration, automatic upgrade/rollback/push, and M12+ remain excluded. The candidate is unaccepted pending independent read-only review and owner acceptance.
...
> **STATUS: ACCEPTED PLAN - M1A-M10 COMPLETE AND ACCEPTED; M11 OPERATIONS/PACKAGING CANDIDATE ACTIVE, LOCAL-ONLY, AND UNACCEPTED**
>
...
> **M1D STATUS (2026-07-20):** M1D Redaction Library is complete and accepted after independent review PASS, with 364 tests and typecheck passing. It remains pure shared denylist, detector, redaction-contract, corpus-test, and export work; runtime context/capture integration remains deferred. Redaction is intentionally conservative and best-effort; future policy expansion may add unsupported secret patterns. M1E and M1F are accepted; M2 and later remain unauthorized.

> **M1E STATUS (2026-07-21):** M1E Capture and Projection Contracts passed final independent read-only review and were accepted by Owner/Bantay, with 376 tests and successful typecheck. It remains pure shared contract work only. Snapshot-shaped values receive structural validation only; a future authoritative runtime store must independently load and establish authority and custody. M1E does not implement or authorize runtime capture, snapshot loading, persistence, filesystem or Bridge Log writing, storage, archives, adapters, database, UI, network, deployment, or production behavior. M1F is accepted; M2 and later remain not started and unauthorized.

> **M1F STATUS (2026-07-21):** M1F Adapter & Coordination Contracts are complete and accepted after comprehensive independent Claude Sonnet 4.6 review PASS, with 395 passing tests and successful typecheck. It remains restricted to pure shared schemas, parsers, evaluators, canonicalization/hash helpers, and synthetic contract tests. Adapter execution, routing, runtime leases and quotas, event streams, journal persistence, telemetry, Bridge, database, filesystem, UI, and network behavior remain deferred. M1A-M1F are accepted; M2 and later remain not started and unauthorized pending a separate explicit owner GO. Deferred LOW hardening: re-evaluate `readOnlyPaths` wildcard overlap with `generatedArtifactRoot` before runtime write-scope enforcement; it is non-blocking because `readOnlyPaths` grants no write authority and does not authorize runtime work.
...
>
> **M1B STATUS: COMPLETED AND MERGED (D-023).** The pure protocol contracts are present in `main` through `2dc6a12`. **M1C Approval-Security Contracts are complete and accepted on `main` after explicit owner/Bantay approval and independent security re-review PASS; accepted commits are `769677dd7ee876a4ecbda08bb3674b6fa8a9a82a` and `cdffc0170facedb4c9791619a0adabc86e7f6f50`.** Pilot-project selection and Obsidian-vault location are deferred inputs, not blockers.
...
- The owner accepted decisions D-001 … D-027 (recorded in [DECISIONS.md](DECISIONS.md)), including all former proposals P-006 … P-018 and the architecture-alignment clarifications.
- Worker authority and documentation policy were reconciled (D-027); no workers may edit the same files concurrently.
...
**STOP POINT (status aligned 2026-07-21):** M1A through M1F are completed and accepted. **M2 and all later milestones are not started or authorized and require a separate explicit owner GO.**
...
2. `packages/shared`: Zod schemas for task states/transitions, commands, WS protocol (client↔CP and CP↔bridge), worker manifest, capability grants, capture records, Bridge Log front matter, adapter/coordination contracts, and the shared secret-detector library; unit tests. **(Split into bounded M1A–M1F subtasks — see below; the assigned implementation worker receives exactly one at a time.)**
...
**Assigned workers:** Codex (current primary implementer in bounded tasks), BUNSO (architecture conformance input and independent review only when assigned), Antigravity (real-PC validation after capability proof), Bantay (security-relevant diff review: auth, grants, redaction, bridge), and Owner (approval at each milestone).

**Suggested milestone order (each a bounded Codex task with its own review; Codex receives exactly one at a time, each dispatched only after owner approval of the previous):** M1A–M1E shared contracts → **M1F Adapter & Coordination Contracts** → M2 control-plane skeleton + DB + auth + WS → M3 bridge enrollment + operation journal + supervisor + managed clone/worktrees → M4 orchestrator + grants end-to-end with a fake "echo worker" → M5 Codex CLI adapter + manual relay (owner-attested import flow) → M6 web app chat + approval flow (CSP and sanitized rendering from the start) → M7 capture/diff/tests/review package → M8 Bridge Log projector + emergency stop + hardening pass → **M9 (separately gated): the explicit apply-to-project action** — apply/cherry-pick/export of an approved patch into the owner's real project as its own displayed bounded action with its own approval card; may be deferred to Phase 3 at the owner's choice, with manual patch application as the interim.
...
Status (aligned 2026-07-21): Phase 0B completed with a conditional pass and owner review. **M1A Core Contracts, M1B Protocol Contracts, M1C Approval-Security Contracts, M1D Redaction Library, M1E Capture and Projection Contracts, and M1F Adapter & Coordination Contracts are completed and accepted; M1F passed comprehensive independent review with 395 tests and successful typecheck.** Redaction remains intentionally conservative and best-effort, with policy expansion deferred. M2 and later remain not started and unauthorized pending a separate explicit owner GO.
...
- **M1F — Adapter & Coordination Contracts:** adapter readiness and run records; assignment, write-scope, lease, handoff, quota-confidence, evidence-taxonomy, and operation-journal reconciliation schemas; proposed `no-eligible-worker` and `stale-lease` blocked reasons. **Contract tests only** — no bridge, routing, adapter, or network implementation. It follows M1E and must complete before M2.

M1F freezes the coordination boundary before runtime orchestration. Its acceptance suite specifies duplicate dispatch/event, stale lease, malformed structured output, expired grant/authentication, cancellation and resume, rate-limit, partial-artifact, crash, and journal-reconciliation cases; runtime proofs occur in M2/M3 and Phase 3. SQLite WAL and the operation journal remain the MVP persistence strategy; a durable workflow engine is not introduced without operational evidence.

### M1C GO prerequisites

The M1C owner GO named its bounded scope and Codex assignment, confirmed M1B merge evidence and a clean non-concurrent workspace, and required Bantay review before the next milestone. M1C acceptance does not authorize M1D, M1E, M1F, runtime code, adapter work, remote access, production access, or deployment; each next milestone requires a separate explicit GO.
...
| 1 | Phase 0B passed, owner reviewed the validation report, and each bounded M1 task received its own GO — **M1A through M1F are complete and accepted; M2 follows only after a separate explicit owner GO** | Live demo + acceptance list + security reviews |
